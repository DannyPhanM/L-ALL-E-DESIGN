  ...
  customElements.define(
    ...
    class ProductForm extends HTMLElement {
      constructor() {
        ...
        if (
          this.closest(".popup-wrapper__quick-view") &&
          !document.querySelector("body").className.includes("hidden")
        )
      ...
      onSubmitHandler(evt) {
        ...
        // Check if "Custom Size" variant is selected (checking both inside and outside the form via form attribute)
        const formId = this.form.getAttribute("id");
        const isCustomSize = Array.from(
        ...
        if (!this.validateCustomProperties()) {
          this.handleErrorMessage(
            `${window.variantStrings.customPropertyError}`,
            true
          );
          return false;
        }
      ...
      validateCustomProperties() {
        ...
        const customInputs = productSection
          ? Array.from(
              productSection.querySelectorAll(`.custom-options [aria-required]`)
            )
          : [];
    ...
    class CustomOptions extends HTMLElement {
      constructor() {
        ...
        if (this.requiredInputs.length > 0)
          Array.from(this.requiredInputs).forEach((requiredInput) =>
            this.validateError(requiredInput)
          );
      ...
      connectedCallback() {
        ...
        if (this.optionType && this.optionType === "color-picker")
          this.setColorPicker();
        if (this.optionType && this.optionType === "image-upload")
          this.setDragAndDrop();
      ...
      checkCustomOptionsValidity() {
        ...
        const dynamicCheckoutWrap = productSection?.querySelector(
          ".shopify-payment-button"
        );
        const requiredInputs = Array.from(
          productSection.querySelectorAll("[aria-required]")
        );
        const hasInvalid = requiredInputs.some((input) => {
          return (
            !input.closest(".product-popup-modal__content") &&
            (input.value === "" ||
              (input.type === "checkbox" && !input.checked))
          );
        });
        if (dynamicCheckoutWrap) {
          dynamicCheckoutWrap.classList.toggle(
            "shopify-payment-button--disable",
            hasInvalid
          );
        }
        return !hasInvalid;
      ...
      setColorPicker() {
        ...
        colorInput.addEventListener("change", (evt) => {
          ...
          const selectedColorStatus = this.querySelector(
            ".selected-color-label"
          );
          const customField = this.querySelector(".color__swatch--color");
          if (selectedColorStatus)
            selectedColorStatus.textContent = currentInput.value;
          if (customField) {
      ...
      validateError(requiredInput) {
        ...
        if (
          requiredInput.tagName === "SELECT" ||
          requiredInput.type === "checkbox" ||
          requiredInput.type === "file"
        ) {
          eventName = "change";
        }

        this.productForm = productSection.querySelector("product-form");
        this.dynamicCheckoutWrap = productSection.querySelector(
          ".shopify-payment-button"
        );
        ...
        requiredInput.addEventListener(eventName, (event) => {
          ...
          if (currentInput.classList.contains("invalid")) {
            if (currentInput.classList.contains("checkbox-group__input"))
              return;
            if (currentInput.type == "checkbox" && !currentInput.checked)
              return;
            removeErrorStyle(currentInput);
            let activeErrors = productSection.querySelectorAll(".invalid");

            if (activeErrors.length === 0) {
              this.dynamicCheckoutWrap.classList.remove(
                "shopify-payment-button--disable"
              );
      ...
      formatFileType(file) {
        ...
        if (type.startsWith("image/")) {
          ...
          if (applicationType[handleVideoSubtype])
        ...
        reader.addEventListener(
          ...
          () => {
            ...
            let thumbnailElement =
              dropZoneWrapElm.querySelector(".drop-zone__thumb");
            ...
            if (!thumbnailElement) {
              ...
              fileInfoText.setAttribute(
                "class",
                "dd-file-info__text flex--column"
              );
              const spanTitle = document.createElement("span");
              const spanFileType = document.createElement("span");
              spanTitle.setAttribute("class", "dd-file-info__title");
              spanFileType.setAttribute(
                "class",
                "dd-file-info__type label_font"
              );

              const btnRemove = document.createElement("button");
              const btnText = this.querySelector(
                ".file-upload__remove"
              ).innerHTML;
            ...
            spanFileType.textContent = `${this.calculateSize(
              file
            )}, ${this.formatFileType(file)}`;
            ...
            if (/\.(jpe?g|png|gif|webp)$/i.test(file.name)) {
              preview.setAttribute(
                "style",
                `background-image:url("${reader.result}"); width: 60px;`
              );
            } else {
  ...
  customElements.define(
    ...
    class ProductCheckboxGroup extends HTMLElement {
      constructor() {
        ...
        const errorMessage = customOptionElm.querySelector(
          ".input-error-message"
        );
        ...
        inputElms.forEach((inputElm) => {
          inputElm.addEventListener("change", (evt) => {
            ...
            if (minLimit && checkedInputs.length >= minLimit) {
              ...
              const requiredInputs = Array.from(
                productSection.querySelectorAll(
                  "input[aria-required]:not(.checkbox-group__input)"
                )
              );
              let checker = (arr) => arr.every((v) => v === true);
              const checkValues = requiredInputs.map((requiredInput) => {
                if (
                  requiredInput.value == "" ||
                  (requiredInput.type == "checkbox" && !requiredInput.checked)
                ) {
                  return false;
                }
                return true;
              ...
              if (areValidInputs && this.dynamicCheckoutWrap) {
                ...
                this.dynamicCheckoutWrap.classList.add(
                  "shopify-payment-button--disable"
                );
              }
            } else if (
              minLimit &&
              checkedInputs.length < minLimit &&
            ...
            if (maxLimit) {
              ...
              const uncheckedInputs = this.querySelectorAll(
    ...
    class TooltipComponent extends HTMLElement {
      ...
      connectedCallback() {
        ...
        const animationDuration =
          getComputedStyle(root)
            ...
            ?.trim() || 0;
        const sideMargin = "16px";

        this.tooltipParent = this.closest(".form__label");
        this.animationDurationValue = parseFloat(animationDuration) * 1000;
        this.sideMarginValue = parseFloat(sideMargin);
      ...
      disconnectedCallback() {
        if (this.tooltipContent) {
          this.tooltipContent.removeEventListener(
            "transitionstart",
            this.onTransitionStart
          );
          this.tooltipContent.removeEventListener(
      ...
      onTransitionStart(e) {
        ...
        if (this.matches(":hover")) {
          ...
          this.adjustTooltipPosition(this.tooltipContent, this.sideMarginValue);
        }
      }
      ...
      onTransitionEnd(e) {
        ...
        const opacityNow = parseFloat(
          getComputedStyle(this.tooltipContent).opacity || "0"
        );
        if (opacityNow === 0) {
      ...
      adjustTooltipPosition(tooltipContent, sideMargin) {
        ...
        if (rect.left < 0) {
          ...
          tooltipContent.style.transform = `translateX(${
            shift + sideMargin
          }px)`;
